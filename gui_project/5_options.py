...
def merge_image():
    try:
        ...
        if img_width > -1:
            image_sizes = [(int(img_width),int(img_width * x.size[1] /x.size[0])) for x in images ]#width 값을 변경 
        else:
            # 원본 사이즈 사용
...
# 1. 가로 넓이 옵션
#가로 넓이 레이블 
lb1_width = Label(frame_option,text="가로넓이",width=8)
lb1_width.pack(side="left",padx=5,pady=5)
#가로 넓이 콤보
...
lb1_space.pack(side="left",padx=5,pady=5)
...
lb1_format.pack(side="left",padx=5,pady=5)
...
#진행 상황 progress bar
frame_progeress = LabelFrame(root,text="진행상황")
frame_progeress.pack(fill="x",padx=5,pady=5,ipady=5)

p_var = DoubleVar()
progress_bar = ttk.Progressbar(frame_progeress, maximum=100, variable= p_var)
