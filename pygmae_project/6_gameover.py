...
charactor_y_pos = screen_height - charactor_height - stage_height
...
#게임 종료 메세지 
# TimeOut(시간 초과) , Mission Complete (성공), Game Over (캐릭터가 공에 맞음 )
...
while running:
    ...
    if charactor_x_pos > screen_width - charactor_width:
        charactor_x_pos = screen_width - charactor_width
    elif charactor_x_pos < 0:
        charactor_x_pos = 0 
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        break #안쪽 for에서 break를 만나면 여기로 진입 가능. 2중 for문을 한번에 탈출       
    ...
    screen.blit(charactor, (charactor_x_pos, charactor_y_pos)) #캐릭터 그리기
    
    #경과 시간 계산
    elepsed_time = (pygame.time.get_ticks() - start_ticks) /1000
    timer = game_font.render("Time : {0}".format(str(int(total_time - elepsed_time))), True, (255,255,255))
    screen.blit(timer, (10,10))

    #시간이 초과 했다면
    if total_time -elepsed_time <= 0:
