...
# 파일 경로에서 폴더 정보 가져오기
print(os.path.dirname(r"C:\Users\전현우\Desktop\python workspace\my_file.txt"))
...
# print(ctime)
# # 날짜 정보를 strftime 을 통해서 연원일 시분초 형태로 출력 
...
# print(os.listdir("rpa_basic")) # 주어진 폴더 밑에서 모든폴더, 파일 목록 가져오기

# 파일 목록 가져오기 (하위 폴더 모두 포함)
# result = os.walk(".") # 주오진 폴더 밑에 있는 모든 폴더, 파일 목록 가져오기
# # print(result)
...
# name = "11_file_sysmtem.py"
# result = []
# for root, dirs, files in os.walk("."):
#     # [a.txt, b.txt, c.txt, 11_file_sysmtem.py, ...]
#     if name in files:
...
#     # [a.txt, b.txt, c.txt, 11_file_sysmtem.py, ...]
#     for name in files:
#         if fnmatch.fnmatch(name, pattern): # 이름이 패터관 일치하면
#             result.append(os.path.join(root, name))
...
# print(os.path.isfile("run_btnnnnnn.png")) # False
...
# 파일명 변경하기
# os.rename("new_file.txt","new_file_renmae.txt") # 이름변경 

# 파일 삭제하기
# os.remove("new_file_renmae.txt")

# 폴더 만들기
# os.mkdir("new_folder")
# os.mkdir("c:/users/전현우/test") #절대 경로 기준으로 폴더 생성 

# os.makedir("new_folder/a/b/c") # 실패 : 하위 폴더 가지는 폴더 생성 시도
# os.makedirs("new_folders/a/b/c") # 성공 : 하위 폴더 가지는 폴더 생성 
...
# 모든 파일이 삭제될 수 있으므로 주위!!

# 파일 복사하기    
# 어떤 파일을 폴더 안으로 복사하기
#shutil.copy("run_btn.png", "test_folder") # 원본 파일 경로 , 대상 폴더 경로 
# 어떤 파일을 폴더 안에 새로운 파일 이름으로 복사하기 
#shutil.copy("run_btn.png", "test_folder/copied_run_btn_.png") # 원본 파일 경로 , 대상 폴더 경로(변경된 파일명까지)
#shutil.copyfile("run_btn.png", "test_folder/copied_run_btn_2.png") # 원본 파일 경로 , 대상 폴더 경로(변경할 파일명까지 있아야 함)
